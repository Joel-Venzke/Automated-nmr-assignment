...
Knowledge of a protein’s structure allows for further understanding of the function of a protein and alterations to its function that can lead to disease. The post-genomic era has brought the need to gather functional and structural information of unknown proteins that are encoded by newly discovered genes. Through the implementation of Nuclear magnetic resonance (NMR) spectroscopy, the structure of molecules, such as proteins, can be studied. However, NMR spectroscopy generates large amounts of data that needs to be assigned. The assignment process can take months to complete and is prone to error. To allow for major advances in structural biology, a faster and more accurate method needs to be developed. Our research focuses on developing an algorithm to automate the assignment process, in an attempt to minimize human error and accelerate the time consuming task.
...
\subsection{Results}
\label{sub:results}
Our algorithm has been tested with real NMR datasets of varying size. When compared to hand-ordered data, the algorithm produces the same, optimal ordering of amino acids. Initially, we focused our research on accurate assignment at the cost of speed. Thus, at the start of our research we could assign only smaller datasets up to ten to fifteen amino acids long. As we shifted our focus to optimizing the algorithm's speed while maintaining the accuracy we managed to achieve we improved runtime to correctly assign up to forty to fifty amino acids. In future research, we plan to test with larger datasets, maximizing speed and efficiency to the point that our algorithm can assign full protein sequences. We also plan to look into testing datasets with more than a few missing datapoints to verify that our algorithm correctly places amino acids when given less-perfect data.

\subsection{Future Goals} % (fold)
\label{sub:future_goals}
We are currently looking into the possibility of parallelizing our algorithm. If possible, our algorithm will be able to handle full NMR datasets. We are also looking to use machine learning in order to optimize the cost calculation. 
