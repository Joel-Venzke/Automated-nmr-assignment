...
// calculates (n!)/(d!)
__host__ __device__ int fact_over_fact(int numerator, int denominator) {
	int total = 1;
...
__host__ __device__ int start_level_value(int numberOfTiles, int currentLevel){
	int total = 0;
...
__host__ __device__ int generate_level(int numberOfTiles, int currentValue) {
...
__host__ __device__ int get_width(int numberOfTiles, int currentLevel) {
...
__host__ __device__ int up_one_level(int numberOfTiles, int currentLevel, int currentValue) {
...
__host__ __device__ int up_one_level(int numberOfTiles, int currentValue) {
...
__host__ __device__ int down_level_first(int numberOfTiles, int currentValue, int numberOfLevelsDown = 1) {
	int currentLevel = generate_level(numberOfTiles, currentValue);
...
__host__ __device__ int down_level_last(int numberOfTiles, int currentValue, int numberOfLevelsDown = 1) {
	int currentLevel = generate_level(numberOfTiles, currentValue);
...
__host__ __device__ int* generate_path(int numberOfTiles, int currentLevel, int currentValue){
...
// see overloaded function
__host__ __device__ int* generate_path(int numberOfTiles, int currentValue){
	int currentLevel = generate_level(numberOfTiles, currentValue);
	int *path = new int[currentLevel+1];
...
__host__ __device__ int* generate_index(int numberOfTiles, int currentLevel, int currentValue) {
	int temp;
...
// see overloaded function
__host__ __device__ int* generate_index(int numberOfTiles, int currentValue) {
	int currentLevel = generate_level(numberOfTiles, currentValue), temp;
	int *placedTiles = generate_path(numberOfTiles, currentLevel, currentValue);
	int *unplacedTiles = new int[numberOfTiles];

	// fill array with possible tile indexes
	for (int j = 0; j < numberOfTiles; ++j)
	{
		unplacedTiles[j] = j;
	}
